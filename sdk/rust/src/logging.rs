//! Configurable log rotation and compression, plus the plugin's tracing setup.
//!
//! Supports time-based rotation (hourly/daily/weekly/never), optional size-based
//! rotation, and gzip compression of rotated files.
//!
//! This lived as a private `logging.rs` copied into eleven plugins — ~377 lines
//! each, ~4,000 in total. The copies had drifted, but only in format-arg style
//! and the plugin name in doc comments: identical public API, identical
//! behaviour. Eleven copies of a rotation-and-retention implementation is
//! eleven places to fix a bug that deletes the wrong log file, so it lives here
//! now and the plugins call [`init_logging`].
//!
//! **Config:**
//! ```toml
//! [logging]
...
// Re-exported so plugins need only this module: `LogLevelHandle` is what
// `main.rs` threads into the management protocol for dynamic level changes, and
// `with_noise_suppression` is used by `init_logging` below.
...
///
/// Also returns a [`crate::logging::LogLevelHandle`] for dynamic log level changes
...
pub fn init_logging(
    ...
    crate::logging::LogLevelHandle,
    crate::mqtt_log_layer::MqttLogHandle,
) {
    ...
    let initial_directives = crate::logging::with_noise_suppression(&initial_directives);
    ...
    let (mqtt_layer, mqtt_handle) = crate::mqtt_log_layer::MqttLogLayer::new();
    ...
    let level_handle =
        crate::logging::LogLevelHandle::from_reload_handle(reload_handle, initial_directives);
