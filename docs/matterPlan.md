...
- Phase 2: next
  - In progress: expanded runtime simulation bootstrap to multiple device types (light/lock/cover) and added bridge admin command API baseline (`list_endpoints`, `get_endpoint`, `get_bridge_metrics` with paging/filter and structured error responses) for API/UI inventory and diagnostics use.
