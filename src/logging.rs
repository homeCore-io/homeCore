//! Configurable log rotation and compression for hc-roku.
//!
...
pub struct LoggingConfig {
    ...
    /// Accepts any `tracing` filter directive, e.g. `"debug"` or `"hc_roku=debug,rumqttc=warn"`.
...
/// - `prefix`: log file name prefix, e.g. `"hc-roku"` → `logs/hc-roku.log`.
/// - `stderr_default`: filter used for stderr when `RUST_LOG` is unset and
///   `cfg.level` is the default `"info"`, e.g. `"hc_roku=info"`.
