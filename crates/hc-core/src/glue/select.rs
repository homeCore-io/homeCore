...
pub async fn handle_cmd(state: &StateStore, pub_bus: &EventBus, device_id: &str, payload: &[u8]) {
    let value: Value = match serde_json::from_slice(payload) {
        Ok(v) => v,
        Err(_) => { warn!(%device_id, "Select: invalid JSON"); return; }
    };
    ...
    let cmd = match value.get("command").and_then(|v| v.as_str()) {
        Some(c) => c.to_string(),
        None => {
            if value.get("option").is_some() || value.get("selected").is_some() { "select".to_string() }
            else { warn!(%device_id, "Select: missing command"); return; }
        }
    };

    let dev = match state.get_device(device_id).await {
        Ok(Some(d)) => d,
        Ok(None) => { warn!(%device_id, "Select: device not found"); return; }
        Err(e) => { warn!(%device_id, error = %e, "Select: read failed"); return; }
    };

    let current = dev.attributes.get("selected").and_then(|v| v.as_str()).unwrap_or("").to_string();
    let options: Vec<String> = dev.attributes.get("options")
    ...
    let new_val = match cmd.as_str() {
        ...
        _ => { warn!(%device_id, %cmd, "Select: unknown command"); return; }
    };
