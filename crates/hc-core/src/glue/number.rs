...
pub async fn handle_cmd(state: &StateStore, pub_bus: &EventBus, device_id: &str, payload: &[u8]) {
    let value: Value = match serde_json::from_slice(payload) {
        Ok(v) => v,
        Err(_) => { warn!(%device_id, "Number: invalid JSON"); return; }
    };
    ...
    let cmd = match value.get("command").and_then(|v| v.as_str()) {
        ...
        None => {
            ...
            else { warn!(%device_id, "Number: missing command"); return; }
        }
    };

    let dev = match state.get_device(device_id).await {
        Ok(Some(d)) => d,
        Ok(None) => { warn!(%device_id, "Number: device not found"); return; }
        Err(e) => { warn!(%device_id, error = %e, "Number: read failed"); return; }
    };

    let current = dev.attributes.get("value").and_then(|v| v.as_f64()).unwrap_or(0.0);
    let step = dev.attributes.get("step").and_then(|v| v.as_f64()).unwrap_or(1.0);
    ...
    let new_val = match cmd.as_str() {
        "set" => clamp(value.get("value").and_then(|v| v.as_f64()).unwrap_or(current)),
        "increment" => clamp(current + step),
        "decrement" => clamp(current - step),
        _ => { warn!(%device_id, %cmd, "Number: unknown command"); return; }
    };
