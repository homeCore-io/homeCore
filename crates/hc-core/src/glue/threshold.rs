...
pub async fn recalculate(state: &StateStore, pub_bus: &EventBus, device_id: &str) {
    let dev = match state.get_device(device_id).await {
        ...
        Ok(None) => return,
        Err(e) => {
            warn!(%device_id, error = %e, "Threshold: read failed");
            return;
        }
    };

    let source_id = match dev
        .attributes
        ...
        .and_then(|v| v.as_str())
    {
        Some(s) => s.to_string(),
        None => {
            warn!(%device_id, "Threshold: no source_device_id");
            return;
        }
    };
    let source_attr = dev
...
pub async fn is_source(
    state: &StateStore,
    threshold_device_id: &str,
